 ...
 * */
Class Spotlights extends CI_Controller {
	...
	public function index() {
		$config = array();
		$config["base_url"] = site_url('admin/spotlights/');
		$config["total_rows"] = $this -> db -> count_all('spotlights');
		$config["per_page"] = 10;
		...
		if ($this -> form_validation -> run() == FALSE)// validation hasn't been passed
		{
			...
			if (strlen($fileUpload) > 0) {
				...
				if ($this -> upload -> do_upload()) {
					$data = $this -> upload -> data();
					$form_data = array('description' => set_value('description'), 'image' => $data['file_name']);
					if ($this -> spotlights_model -> Update($id, $form_data) == TRUE)// the information has therefore been successfully saved in the db
					{
						$this -> ci_alerts -> set('success', 'Saved Successfully');
	...
	function delete($id = null, $filename = null) {
		...
		if ($id == '' || $id == null) {
			...
			$this -> db -> delete('spotlights', array('id' => $id));
