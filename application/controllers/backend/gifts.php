...
Class Gifts extends CI_Controller {
	...
	public function index() {
		...
		$data['gifts'] = $this -> gifts -> GetAll($config["per_page"], $page);
		$data['links'] = $this -> pagination -> create_links();
		$this -> load -> view('backend/gifts/list_view', $data);
	...
	function add() {
		...
		{
			$this -> load -> view('backend/gifts/add_view');
		} else// passed validation proceed to post success logic
		{
			...
			if (strlen($fileUpload) > 0) {
				...
				if ($this -> upload -> do_upload()) {
					...
					$form_data = array('name' => $this -> input -> post('name'), 'description' => $this -> input -> post('description'), 'image' => $data['file_name']);
					if ($this -> gifts -> SaveGifts($form_data) == TRUE)// the information has therefore been successfully saved in the db
					{
						$this -> ci_alerts -> set('success', 'Saved Successfully');
						redirect('admin/gifts/add');
					...
					$this -> load -> view('backend/gifts/add_view', $data);
				}
	...
	function edit($id) {
		...
		if ($this -> form_validation -> run() == FALSE)// validation hasn't been passed
		{
			...
			$this -> load -> view('backend/gifts/edit_view', $data);
		} else {// passed validation proceed to post success logic
			...
			if (strlen($fileUpload) > 0) {
				...
				if ($this -> upload -> do_upload()) {
					$data = $this -> upload -> data();
					$form_data = array('name' => $this -> input -> post('name'), 'description' => $this -> input -> post('description'), 'image' => $data['file_name']);
					if ($this -> gifts -> UpdateGifts($id, $form_data) == TRUE)// the information has therefore been successfully saved in the db
					{
						$this -> ci_alerts -> set('success', 'Saved Successfully');
						redirect('admin/gifts/edit/' . $id);
						// or whatever logic needs to occur
					} else {
						$this -> ci_alerts -> set('success', 'An error occurred saving your information. Please try again later');
						redirect('admin/gifts/edit/' . $id);
					...
					//Failed to upload file.
					$data['upload_error'] = $this -> upload -> display_errors();
					$data['testimonials'] = $this -> gifts -> GetOne($id);
	...
	function delete($id = null, $filename = null) {
		...
		if ($id == '' || $id == null) {
			...
			$this -> db -> delete('gifts', array('id' => $id));
