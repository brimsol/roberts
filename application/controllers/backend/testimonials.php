...
Class Testimonials extends CI_Controller {
	...
	public function index() {
		...
		$this -> load -> view('backend/testimonials/list_view',$data);
	...
	function add() {
		...
		{
			$this -> load -> view('backend/testimonials/add_view');
		} else// passed validation proceed to post success logic
		{
			...
			if ($this -> upload -> do_upload()) {
				...
				if ($this -> testimonials_model -> SaveTestimonial($form_data) == TRUE)// the information has therefore been successfully saved in the db
				{
				...
				$this -> load -> view('backend/testimonials/add_view', $data);
			}
	...
	function edit($id) {
		...
		if ($this -> form_validation -> run() == FALSE)// validation hasn't been passed
		{
			$data['testimonials'] = $this -> testimonials_model -> GetOne($id);
			$this -> load -> view('backend/testimonials/edit_view',$data);
		} else {// passed validation proceed to post success logic
			...
			if (strlen($fileUpload) > 0) {
				...
				if ($this -> upload -> do_upload()) {
					$data = $this -> upload -> data();
					$form_data = array('name' => set_value('name'), 'description' => set_value('description'), 'image' => $data['file_name']);
					if ($this -> testimonials_model -> UpdateTestimonial($id, $form_data) == TRUE)// the information has therefore been successfully saved in the db
					{
						$this -> ci_alerts -> set('success', 'Saved Successfully');
